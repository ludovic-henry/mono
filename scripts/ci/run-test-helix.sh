...
EOF
    ...
    ${TESTCMD} --label=upload-to-helix --timeout=5m --fatal msbuild helix.proj -t:RunCloudTest -p:HelixApiAccessKey=${MONO_HELIX_API_KEY} -p:CloudDropAccountKey=${MONO_HELIX_CLOUDDROUP_ACCOUNTKEY} -p:TargetQueues=debian.9.amd64.open -p:BuildMoniker=$(git rev-parse HEAD) -p:HelixArchLabel=${helix_arch_label} -p:HelixConfigLabel=Mainline
