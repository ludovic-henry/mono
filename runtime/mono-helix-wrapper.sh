...
if [ "$1" = "run-bcl-tests" ]; then
    if [ "$2" = "xunit" ]; then
        ...
        MONO_REGISTRY_PATH="$HOME/.mono/registry" MONO_TESTS_IN_PROGRESS="yes" "${MONO_EXECUTABLE}" --config "$r/runtime/etc/mono/config" --debug nunit-lite-console.exe "tests/$3" -format:nunit2 -result:testResult.xml
        exit $?
    else
