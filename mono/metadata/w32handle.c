...
/* Make MonoW32HandleSlot 8kB */
#define HANDLES_PER_SLOT ((8192 - /* MonoW32HandleSlot.next */ sizeof(MonoW32HandleSlot*)) / sizeof(MonoW32Handle))
...
static MonoW32HandleSlot *handle_slots_first;
static MonoW32HandleSlot *handle_slots_last;
...
static MonoW32HandleSlot*
w32handle_slot_alloc (void)
{
	MonoW32HandleSlot *slot;

	slot = mono_valloc (NULL, sizeof(MonoW32HandleSlot), MONO_MMAP_READ|MONO_MMAP_WRITE, MONO_MEM_ACCOUNT_W32HANDLE);
...
static void
w32handle_slot_free (MonoW32HandleSlot* slot)
{
	mono_vfree (slot, sizeof(MonoW32HandleSlot), MONO_MEM_ACCOUNT_W32HANDLE);
}
...
mono_w32handle_init (void)
{
	...
	handle_slots_first = handle_slots_last = w32handle_slot_alloc ();
...
mono_w32handle_cleanup (void)
{
	...
	for (slot = handle_slots_first; slot; slot = slot_next) {
		slot_next = slot->next;
		w32handle_slot_free (slot);
	}
}
...
retry:
	...
	if (!retried) {
		...
		slot = handle_slots_first;
		index = 0;
		retried = TRUE;
		goto retry;
	}

	handle_slots_last = (handle_slots_last->next = w32handle_slot_alloc ());
