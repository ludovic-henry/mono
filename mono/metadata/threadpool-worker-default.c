...
worker_try_create (void)
{
	...
	thread = mono_thread_create_internal (mono_get_root_domain (), worker_thread, NULL, TRUE, FALSE, 0, &error);
	if (!thread) {
	...
	for (;;) {
		...
		case MONITOR_STATUS_NOT_RUNNING:
			...
			if (InterlockedCompareExchange (&worker.monitor_status, MONITOR_STATUS_REQUESTED, MONITOR_STATUS_NOT_RUNNING) == MONITOR_STATUS_NOT_RUNNING) {
				...
				if (!mono_thread_create_internal (mono_get_root_domain (), monitor_thread, NULL, TRUE, FALSE, SMALL_STACK, &error)) {
