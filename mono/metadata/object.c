...
mono_class_create_runtime_vtable (MonoDomain *domain, MonoClass *klass, gboolean raise_on_error)
{
	...
	if (class_size) {
		...
		if (klass->has_static_refs) {
			...
			if (statics_gc_descr == 0x5c0001)
				g_print ("statics desc %p for %s.%s (vtable = %p)\n", (gpointer)(gsize) statics_gc_descr, klass->name_space, klass->name, vt);
...
mono_handle_string_new_size (MonoDomain *domain, gint32 len)
{
	...
	/* check for overflow */
	if (len < 0 || len > ((SIZE_MAX - G_STRUCT_OFFSET (MonoString, chars) - 8) / 2))
		mono_gc_handle_out_of_memory (-1);
	...
	return mono_gc_handle_alloc_string (vtable, size, len);
}
...
mono_handle_array_new_specific (MonoVTable *vtable, uintptr_t size)
{
	...
	if (!mono_array_calc_byte_len (vtable->klass, size, &byte_size))
		return mono_gc_handle_out_of_memory (MONO_ARRAY_MAX_SIZE);

	return mono_gc_handle_alloc_vector (vtable, byte_size, size);
}
