		...
		MonoObject *target, MonoArray **out_args)
{
	...
	if (target && mono_object_is_transparent_proxy (target)) {
		...
		if (mono_class_is_contextbound (tp->remote_class->proxy_class) && tp->rp->context == (MonoObject *) mono_context_get ()) {
			...
			ret = mono_remoting_invoke ((MonoObject *)tp->rp, this, &exc, out_args);
			if (exc)
	...
	ret = mono_runtime_invoke_array (method, method->klass->valuetype? mono_object_unbox (target): target, this->args, &exc);
	if (exc)
		mono_raise_exception ((MonoException*) exc);
	...
	for (i = 0, j = 0; i < sig->param_count; i++) {
		if (sig->params [i]->byref) {
			MonoObject* arg;
			arg = mono_array_get (this->args, gpointer, i);
