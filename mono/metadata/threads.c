...
mono_thread_execute_interruption (MonoInternalThread *thread)
{
	...
	if (InterlockedCompareExchange (&thread->interruption_requested, FALSE, TRUE)) {
		...
		mono_thread_info_clear_interrupt (mono_thread_info_current ());
	}
...
abort_thread_critical (MonoThreadInfo *info, gpointer ud)
{
	...
	if (!protected_wrapper && running_managed) {
		 ...
		 * make it return.
		 */
		data->interrupt_token = mono_thread_info_prepare_interrupt (mono_thread_info_lookup ((MonoNativeThreadId) (gsize) thread->tid));
		return MonoResumeThread;
	}
}
...
abort_thread_internal (MonoInternalThread *thread, gboolean can_raise_exception, gboolean install_async_abort)
{
	...
	*/
	if (thread == mono_thread_internal_current ()) {
		...
		if (exc)
			mono_raise_exception (exc);
		mono_thread_info_interrupt (mono_thread_info_lookup ((MonoNativeThreadId) (gsize) thread->tid));
		return;
	}
