...
_wapi_thread_apc_pending (gpointer handle)
{
	...
	info = mono_thread_info_lookup (thread->id);
	g_assert (info);

	return InterlockedReadPointer ((gpointer*) &info->interrupt_token) == INTERRUPT_TOKEN;
}
...
wapi_current_thread_desc (void)
{
	...
	thread_info = mono_thread_info_current ();
	interrupt_token = InterlockedReadPointer ((gpointer*) &thread_info->interrupt_token);
	...
	else if (interrupt_token == INTERRUPT_TOKEN)
		g_string_append_printf (text, "interrupted state");
	else
		g_string_append_printf (text, "waiting on %p : %s ", interrupt_token->data, _wapi_handle_typename[_wapi_handle_type (interrupt_token->data)]);
